...
app.get("/api/test-results", async (c) => {
  try {
    ...
    console.log(
      `Prioritizing January 13th data from ${yesterday.toISOString()} to ${endOfToday.toISOString()}`
    );
    ...
    if (rawLogs && rawLogs.length > 0) {
      ...
      rawLogs.forEach((log, index) => {
        ...
        console.log(
          `  ${index + 1}. System: ${system || "NO_SYSTEM"}, Journeys: ${journeyCount}, Run ID: ${log.raw_payload?.run_id}`
        );
      });
    }
    ...
    if (rawLogs && rawLogs.length > 0) {
      ...
      rawLogs.forEach((log, index) => {
        ...
        console.log(
          `  ${index + 1}. Date: ${date}, System: ${system || "NO_SYSTEM"}, Journeys: ${journeyCount}, Run ID: ${log.raw_payload?.run_id}`
        );
      });
    ...
    if (desktopRawLog) {
      ...
      console.log(
        `Found Desktop Site raw log data with ${journeyCount} journeys, run_id:`,
        rawPayload?.run_id
      );
      ...
      latestRun = {
        ...
        passed_journeys:
          rawPayload?.summary?.passed_journeys ||
          journeys.filter((j: any) => j.status === "PASSED").length,
        failed_journeys:
          rawPayload?.summary?.failed_journeys ||
          journeys.filter((j: any) => j.status === "FAILED").length,
    ...
    if (!latestRun) {
      console.log(
        "No Desktop Site raw_test_logs found, checking test_runs table..."
      );
      ...
      if (testRuns && testRuns.length > 0) {
        ...
        testRuns.forEach((run, index) => {
          ...
          console.log(
            `  ${index + 1}. System: ${system || "NO_SYSTEM"}, Journeys: ${run.total_journeys}, Run ID: ${run.run_id}`
          );
        });
      }
      ...
      if (desktopRuns.length > 0) {
        ...
        latestRun = desktopRuns.reduce((max, run) => {
          return (run.total_journeys || 0) > (max.total_journeys || 0)
            ? run
            : max;
        });

        console.log(
          `Found Desktop Site test_run with ${latestRun.total_journeys} journeys:`,
          latestRun.run_id
        );
    ...
    try {
      ...
      if (omsRunData && omsRunData.latestRun) {
        ...
        const passedOmsJourneys = omsJourneys.filter(
          (j: any) => j.status === "PASSED"
        ).length;
        const failedOmsJourneys = omsJourneys.filter(
        ...
        omsJourneys.forEach((j: any) => {
          ...
          passedOmsSteps += steps.filter(
            (s: any) => s.status === "PASSED"
          ).length;
          failedOmsSteps += steps.filter(
            (s: any) => s.status === "FAILED"
          ).length;
        });
        ...
        const omsSuccessRate =
          totalOmsSteps > 0
            ? ((passedOmsSteps / totalOmsSteps) * 100).toFixed(1)
            : "0";
        ...
        console.log("OMS data prepared:", {
          totalJourneys: totalOmsJourneys,
          modulesCount: omsModules.length,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (ppRunData && ppRunData.latestRun) {
        ...
        const passedPpJourneys = ppJourneys.filter(
          (j: any) => j.status === "PASSED"
        ).length;
        const failedPpJourneys = ppJourneys.filter(
          (j: any) => j.status === "FAILED"
        ).length;
        ...
        ppJourneys.forEach((j: any) => {
          ...
          passedPpSteps += steps.filter(
            (s: any) => s.status === "PASSED"
          ).length;
          failedPpSteps += steps.filter(
            (s: any) => s.status === "FAILED"
          ).length;
        });
        ...
        const ppSuccessRate =
          totalPpSteps > 0
            ? ((passedPpSteps / totalPpSteps) * 100).toFixed(1)
            : "0";
        ...
        partnerPanelData = {
          ...
          modules: ppModules,
          overallStatus:
            failedPpJourneys === 0 ? "ALL SYSTEMS GO ✅" : "ISSUES DETECTED ❌",
          isSuccess: failedPpJourneys === 0 && passedPpJourneys > 0,
        };
        console.log("Partner Panel data prepared:", {
          totalJourneys: totalPpJourneys,
          modulesCount: ppModules.length,
        });
      }
    } catch (error) {
    ...
    if (!latestRun || journeys.length === 0) {
      console.log(
        "No Supabase data found in recent days, using mock data for desktop"
      );
    ...
    const desktopModules = journeys
      .map((journey: any) => {
        ...
        allSteps.forEach((step: any, index: number) => {
          ...
          const isJourneyEnd =
            stepName.includes("PNC Created Successfully") ||
          ...
          if (isJourneyEnd || index === allSteps.length - 1) {
            ...
            const passedSteps = currentJourneySteps.filter(
              (s: any) => s.status === "PASSED"
            ).length;
            const failedSteps = currentJourneySteps.filter(
              (s: any) => s.status === "FAILED"
            ).length;
            const journeyDuration = currentJourneySteps.reduce(
              (sum, s) => sum + (s.duration_ms || s.durationMs || 0),
              0
            );
            ...
            if (journeyNumber === 1)
              journeyName = "User Authentication & Product Selection";
            else if (journeyNumber === 2)
              journeyName = "Payment Methods Testing";
            else if (journeyNumber === 3)
              journeyName = "Profile & Address Management";
            else if (journeyNumber === 4)
              journeyName = "International & Advanced Features";
            ...
            journeyBoundaries.push({
              ...
              failureReason:
                failedSteps > 0
                  ? currentJourneySteps.find((s) => s.status === "FAILED")
                ...
                statusIcon:
                  step.status === "PASSED"
                    ...
                    : step.status === "FAILED"
                      ...
                      : "⚪",
                duration:
                  step.duration_ms || step.durationMs || step.duration || 0,
        ...
        if (journeyBoundaries.length === 0) {
          const passedSteps = allSteps.filter(
            (s: any) => s.status === "PASSED"
          ).length;
          ...
          return [
            {
              journeyNumber:
                journey.journey_number || journey.journeyNumber || 1,
              name:
              ...
              totalSteps:
                ...
                0,
              passed:
                passedSteps || journey.passed_steps || journey.passedSteps || 0,
              failed:
                failedSteps || journey.failed_steps || journey.failedSteps || 0,
              duration: Math.round(
                (journey.duration_ms || journey.durationMs || 0) / 1000
              ),
              durationFormatted: formatDuration(
                journey.duration_ms || journey.durationMs || 0
              ),
              failureReason:
                journey.failure_reason || journey.failureReason || null,
              errorType: journey.error_type || journey.errorType || null,
              errorMessage:
                journey.error_message || journey.errorMessage || null,
    ...
    const desktopData = {
      ...
      overallStatus:
        failedJourneys === 0 ? "ALL SYSTEMS GO ✅" : "ISSUES DETECTED ❌",
...
app.get("/api/tab-performance/:system", async (c) => {
  try {
    ...
    if (system !== "OMS" && system !== "PARTNER_PANEL") {
      return c.json(
        { error: "Invalid system. Use 'OMS' or 'PARTNER_PANEL'" },
        400
      );
    }

    const days = parseInt(c.req.query("days") || "7");
    const tabPerformance = await fetchTabPerformance(
      system as "OMS" | "PARTNER_PANEL",
      days
    );
...
app.get("/api/screenshots", async (c) => {
  try {
    ...
    const groupedScreenshots = filteredScreenshots.reduce(
      (acc: any, screenshot: any) => {
...
app.get("/api/screenshots/journey/:runId/:journeyNumber", async (c) => {
  try {
    ...
    console.log(
      `Fetching FAILURE screenshots for journey ${journeyNumber} in run ${runId}`
    );
    ...
    if (error) {
      ...
      return c.json(
        { error: "Failed to fetch journey failure screenshots" },
        500
      );
    }
...
app.get("/api/test-results/:platform", async (c) => {
  ...
  try {
    ...
    if (platform === "desktop") {
      ...
      console.log(
        `Prioritizing January 13th desktop data from ${yesterday.toISOString()} to ${endOfToday.toISOString()}`
      );
      ...
      if (desktopRawLog) {
        ...
        latestRun = {
          ...
          total_journeys:
            rawPayload?.summary?.total_journeys || journeys.length,
          passed_journeys:
            rawPayload?.summary?.passed_journeys ||
            journeys.filter((j: any) => j.status === "PASSED").length,
          failed_journeys:
      ...
      const desktopModules = journeys.map((journey: any) => {
        ...
        return {
          ...
          statusIcon:
            journey.status === "PASSED"
              ...
              : journey.status === "FAILED"
                ...
                : "⚪",
          totalSteps:
            steps.length || journey.total_steps || journey.totalSteps || 0,
          passed:
            steps.filter((s: any) => s.status === "PASSED").length ||
            journey.passed_steps ||
            journey.passedSteps ||
            0,
          failed:
            steps.filter((s: any) => s.status === "FAILED").length ||
            journey.failed_steps ||
            journey.failedSteps ||
            0,
          duration: Math.round(
            (journey.duration_ms || journey.durationMs || 0) / 1000
          ...
          steps: steps.map((step: any, index: number) => ({
            stepNumber: step.step_number || step.stepNumber || index + 1,
            name:
              step.step_name || step.stepName || step.name || "Unknown Step",
            status: step.status,
            statusIcon:
              step.status === "PASSED"
                ? "✅"
      ...
      const successRate =
        totalSteps > 0 ? ((passedSteps / totalSteps) * 100).toFixed(1) : "0";
      const failedJourneys = journeys.filter(
        (j: any) => j.status === "FAILED"
      ).length;
    ...
    if (platform === "oms") {
      ...
      try {
        ...
        if (omsRunData && omsRunData.latestRun) {
          ...
          const passedJourneys = omsJourneys.filter(
            (j: any) => j.status === "PASSED"
          ).length;
          const failedJourneys = omsJourneys.filter(
            (j: any) => j.status === "FAILED"
          ).length;
          ...
          const successRate =
            totalSteps > 0
              ? ((passedSteps / totalSteps) * 100).toFixed(1)
              : "0";
    ...
    if (platform === "android") {
      ...
      try {
        ...
        if (ppRunData && ppRunData.latestRun) {
          ...
          const passedJourneys = ppJourneys.filter(
            (j: any) => j.status === "PASSED"
          ).length;
          const failedJourneys = ppJourneys.filter(
            (j: any) => j.status === "FAILED"
          ).length;
    ...
    const mockResults = generateMockTestResults();
    return c.json(
      mockResults[platform as keyof typeof mockResults] || {
...
app.get("/api/journey/:journeyId", async (c) => {
  ...
  try {
    ...
    const journey =
      journeyData && journeyData.length > 0 ? journeyData[0] : null;
...
app.get("/api/test-runs", async (c) => {
  try {
    ...
    console.log(
      `Fetching today's test runs from ${startOfToday.toISOString()} to ${endOfToday.toISOString()}`
    );
    ...
    if (rawLogs && rawLogs.length > 0) {
      ...
      const transformedRuns = rawLogs.map((log: any) => {
        ...
        return {
          ...
          total_journeys:
            payload?.summary?.total_journeys || payload?.journeys?.length || 0,
...
app.get("/api/health", async (c) => {
  try {
    ...
    console.log(
      `Fetching today's health metrics from ${startOfToday.toISOString()} to ${endOfToday.toISOString()}`
    );
    ...
    if (rawLogs && rawLogs.length > 0) {
      const healthData = {
        ...
        avg_success_rate:
          rawLogs.reduce(
            (sum, r) => sum + (r.raw_payload?.summary?.success_rate || 0),
            0
          ) / rawLogs.length,
        avg_runtime_ms:
          rawLogs.reduce(
            (sum, r) => sum + (r.raw_payload?.total_runtime_ms || 0),
            0
          ) / rawLogs.length,
        last_execution: rawLogs[0].executed_at,
        total_failures: rawLogs.reduce(
          (sum, r) => sum + (r.raw_payload?.summary?.failed_journeys || 0),
          0
        ),
      };
      return c.json(healthData);
    ...
    const healthData = {
      ...
      avg_success_rate:
        recentRuns && recentRuns.length > 0
          ? recentRuns.reduce((sum, r) => sum + (r.success_rate || 0), 0) /
            recentRuns.length
          : 0,
      avg_runtime_ms:
        recentRuns && recentRuns.length > 0
          ? recentRuns.reduce((sum, r) => sum + (r.total_runtime_ms || 0), 0) /
            recentRuns.length
          : 0,
      last_execution:
        recentRuns && recentRuns.length > 0 ? recentRuns[0].executed_at : null,
      total_failures:
        recentRuns?.reduce((sum, r) => sum + (r.failed_journeys || 0), 0) || 0,
    };
  ...
  try {
    ...
    if (rawLogs && rawLogs.length > 0) {
      ...
      const journey = journeys.find(
        (j: any) => (j.journey_number || j.journeyNumber) === journeyNumber
      );
      if (journey) {
